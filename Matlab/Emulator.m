...
d1 = 25;    % distance between tags     [cm]
...
robStart = [30 40]; % Startposition of robot in x, y   [cm]
robEnd = [70 90];   % Endposition of robot in x, y     [cm]

robSpeed = 0.1;       % Speed robot [m/s]
cycleT = 100;        % Cycletime in [ms]

measuementeNumber = num2str(1);
...
% Txt file name
name = ['Meas',measuementeNumber,'.txt'];
fileID = fopen(name,'w');
...
for l=0:cycleT/1000:t1                                
    ...
    for m = 1:numTags                   % m = current number of tag
        ...
        % Store the information in an txt file
    end
    fprintf(fileID,'%6s\n','---------');  
    % toc
    pause(100/1000)
